...
echo "Also ensure that mtm/cosmovisor service is stopped."
echo ""
...
then
  ...
  NODE3_IP="microtick-rpc.3tekos.fr"
  RPC3="https://$NODE2_IP"
  P2P_PORT3=26656
  RPC_PORT3=443
  ...
  echo ""
  echo "##################################################################"
  echo "#     Parameters to change in: .microtick/config/config.toml          #"
  echo "##################################################################"
  echo "#  Temporaly search and replace this params with this values     #"
  echo "##################################################################"
  echo ""
  echo "persistent_peers = \"${NODE1_ID}@${NODE1_IP}:${P2P_PORT1},${NODE2_ID}@${NODE2_IP}:${P2P_PORT2},${NODE3_ID}@${NODE3_IP}:${P2P_PORT3}\""
  echo ""
  echo "Go to -StateSync section-"
  echo "========================="
  echo 'enable = true'
  echo "rpc_servers = \"http://$NODE1_IP:$RPC_PORT1,http://$NODE2_IP:$RPC_PORT2,https://$NODE3_IP:$RPC_PORT3\""
